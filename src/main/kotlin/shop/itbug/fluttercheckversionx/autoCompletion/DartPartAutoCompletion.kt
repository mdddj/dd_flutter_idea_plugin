                ...
                override fun addCompletions(
                    ...
                    files.forEach { file ->
                        ...
                        findChildrenOfAnyType.forEach { lib ->
                            val ele =
                                LookupElementBuilder.create("part of ${lib.libraryNameElement?.text ?: ""}").withIcon(
                                    MyIcons.flutter
                                ).withPsiElement(
                                    MyDartPsiElementUtil.createDartPart(
                                        "part of ${lib.libraryNameElement?.text}",
                                        project
                                    )
                                )
                            p2.addElement(ele)
...
class OnlyProjectFileSearch(private val myProject: Project) : GlobalSearchScope(myProject) {
    ...
    override fun contains(p0: VirtualFile): Boolean {
        ...
        return fileIndexs.isInSourceContent(p0) && PsiTreeUtil.findChildrenOfAnyType(
            psiFile,
